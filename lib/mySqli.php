...
class mySqliDB
{
        ...
        if (empty($fields) && $isAssocArr != 1)
		 ...
		 if (is_array($updates)) {
			 foreach ($updates as $field => $value) {
				 $strUpdates .= "$strComma `$field` = '$value' ";
				 $strComma = ',';
			 }
		 } else
			 $strUpdates .= "$updates";
    ...
    public function update($values, $where = '', $tblName = '')
    {
        ...
        if (is_array($values)) {
            foreach ($values as $field => $value) {
                $strValue .= "$strSaprator `$field` = '$value' ";
                $strSaprator = ',';
            }
        } else
            $strValue .= "$values";
        ...
        if (is_array($where)) {
            foreach ($where as $field => $value) {
                if (is_array($value)) {
                    ...
                    $strWhere .= " $strSeparator `$value[0]` $operator '$value[1]' ";
                    $strSeparator = ' AND ';
                } elseif (is_int($field)) {
                    $strWhere .= "$strSeparator $value";
                    $strSeparator = ' AND ';
                } else {
                    $strWhere .= "$strSeparator `$field`='$value'";
                    $strSeparator = ' AND ';
                }
            }
        } else $strWhere .= $where;
    ...
    function evalOrderByString($orderby)
    {
        ...
        if (is_array($orderby)) {
            foreach ($orderby as $field => $value) {
                if (is_int($field))
                    $strOrder .= "$strSeparator $value";
                else
                    $strOrder .= "$strSeparator $field $value";
                $strSeparator = ',';
            }
